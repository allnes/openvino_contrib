...
ArmGpuPlugin::ExecutableNetwork::ExecutableNetwork(const std::shared_ptr<const ngraph::Function>& function,
...
ArmGpuPlugin::ExecutableNetwork::ExecutableNetwork(std::istream& model,
                                                     const Configuration& cfg,
                                                     const Plugin::Ptr& plugin)
    : _cfg(cfg),
      _plugin(plugin) {
...
void ArmGpuPlugin::ExecutableNetwork::CompileNetwork(const std::shared_ptr<const ngraph::Function>& function,
                                                       const InferenceEngine::InputsDataMap& inputInfoMap,
                                                       const InferenceEngine::OutputsDataMap& outputsInfoMap) {
...
void ArmGpuPlugin::ExecutableNetwork::InitExecutor() {
    ...
    streamsExecutorConfig._name = "ArmGpuStreamsExecutor";
    ...
    // InferenceEngine::ExecutorManager::getInstance()->getIdleCPUStreamsExecutor({"ArmGpuCallbackExecutor"});
}
// ! [executable_network:init_executor]

// ! [executable_network:create_infer_request_impl]
InferenceEngine::IInferRequestInternal::Ptr ArmGpuPlugin::ExecutableNetwork::CreateInferRequestImpl(
    InferenceEngine::InputsDataMap networkInputs,
    InferenceEngine::OutputsDataMap networkOutputs) {
    return std::make_shared<ArmGpuInferRequest>(networkInputs,
                                                  networkOutputs,
...
InferenceEngine::IInferRequestInternal::Ptr ArmGpuPlugin::ExecutableNetwork::CreateInferRequestImpl(
    ...
    return std::make_shared<ArmGpuInferRequest>(inputs,
                                                  outputs,
                                                  std::static_pointer_cast<ExecutableNetwork>(shared_from_this()));
}
// ! [executable_network:create_infer_request_impl]
...
InferenceEngine::IInferRequestInternal::Ptr ArmGpuPlugin::ExecutableNetwork::CreateInferRequest() {
    ...
    return std::make_shared<ArmGpuAsyncInferRequest>(std::static_pointer_cast<ArmGpuInferRequest>(internalRequest),
                                                       _taskExecutor,
...
InferenceEngine::Parameter ArmGpuPlugin::ExecutableNetwork::GetConfig(const std::string& name) const {
    return _cfg.Get(name);
}
// ! [executable_network:get_config]

// ! [executable_network:get_metric]
InferenceEngine::Parameter ArmGpuPlugin::ExecutableNetwork::GetMetric(const std::string& name) const {
    ...
    if (EXEC_NETWORK_METRIC_KEY(SUPPORTED_METRICS) == name) {
        ...
        std::vector<std::string> configKeys = {CONFIG_KEY(DEVICE_ID),
                                               ...
                                               ARM_GPU_CONFIG_KEY(THROUGHPUT_STREAMS)};
...
void ArmGpuPlugin::ExecutableNetwork::Export(std::ostream& modelStream) {
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, "ExecutableNetwork::Export");
