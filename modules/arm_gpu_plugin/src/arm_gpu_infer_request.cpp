...
#include "arm_gpu_executable_network.hpp"
#include "arm_gpu_itt.hpp"
#include "arm_gpu_plugin.hpp"
...
// ! [infer_request:ctor]
ArmGpuInferRequest::ArmGpuInferRequest(const InferenceEngine::InputsDataMap& networkInputs,
                                           const InferenceEngine::OutputsDataMap& networkOutputs,
                                           const std::shared_ptr<ArmGpuPlugin::ExecutableNetwork>& executableNetwork)
    : IInferRequestInternal(networkInputs, networkOutputs),
      _executableNetwork(executableNetwork) {
    createInferRequest();
}

ArmGpuInferRequest::ArmGpuInferRequest(const std::vector<std::shared_ptr<const ov::Node>>& inputs,
...
void ArmGpuInferRequest::createInferRequest() {
    ...
    _profilingTask = {
        openvino::itt::handle("ArmGpu" + std::to_string(_executableNetwork->_cfg.deviceId) + "_" + name +
                              "_Preprocess"),
        openvino::itt::handle("ArmGpu" + std::to_string(_executableNetwork->_cfg.deviceId) + "_" + name +
                              "_Postprocess"),
        openvino::itt::handle("ArmGpu" + std::to_string(_executableNetwork->_cfg.deviceId) + "_" + name +
...
// ! [infer_request:dtor]
ArmGpuInferRequest::~ArmGpuInferRequest() {
    _executableNetwork->_requestId--;
}
// ! [infer_request:dtor]

void ArmGpuInferRequest::allocateDeviceBuffers() {
...
void ArmGpuInferRequest::allocateBlobs() {
...
void ArmGpuInferRequest::InferImpl() {
...
void ArmGpuInferRequest::inferPreprocess() {
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, _profilingTask[Preprocess]);
    auto start = Time::now();
...
void ArmGpuInferRequest::startPipeline() {
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, _profilingTask[StartPipeline])
    auto start = Time::now();
...
void ArmGpuInferRequest::waitPipeline() {
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, _profilingTask[WaitPipeline])
    auto start = Time::now();
...
void ArmGpuInferRequest::inferPostprocess() {
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, _profilingTask[Postprocess]);
    auto start = Time::now();
...
InferenceEngine::Blob::Ptr ArmGpuInferRequest::GetBlob(const std::string& name) {
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, "GetBlob");
...
void ArmGpuInferRequest::SetBlob(const std::string& name, const InferenceEngine::Blob::Ptr& userBlob) {
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, "SetBlob");
    if (name.empty()) {
...
void ArmGpuInferRequest::SetBlobsImpl(const std::string& name, const InferenceEngine::BatchedBlob::Ptr& batchedBlob) {
    _batched_inputs[name] = batchedBlob;
}
// ! [infer_request:set_blobs_impl]

// ! [infer_request:get_performance_counts]
std::map<std::string, InferenceEngineProfileInfo> ArmGpuInferRequest::GetPerformanceCounts() const {
