...
#include "transformations/arm_gpu_pattern_transformation.hpp"
...
Plugin::Plugin() {
    ...
    _waitExecutor =
        InferenceEngine::ExecutorManager::getInstance()->getIdleCPUStreamsExecutor({"ArmGpuWaitExecutor"});
}
// ! [plugin:ctor]
...
Plugin::~Plugin() {
    ...
    InferenceEngine::ExecutorManager::getInstance()->clear("ArmGpuStreamsExecutor");
    InferenceEngine::ExecutorManager::getInstance()->clear("ArmGpuWaitExecutor");
    // NOTE: Uncomment this if Inference Engine Executor cache is used to create callback executor
    // ExecutorManager::getInstance()->clear("ArmGpuCallbackExecutor");
}
// ! [plugin:dtor]
...
InferenceEngine::IExecutableNetworkInternal::Ptr Plugin::LoadExeNetworkImpl(const InferenceEngine::CNNNetwork& network,
    ...
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, "Plugin::LoadExeNetworkImpl");
...
InferenceEngine::IExecutableNetworkInternal::Ptr Plugin::ImportNetwork(
    ...
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, "Plugin::ImportNetwork");
...
InferenceEngine::QueryNetworkResult Plugin::QueryNetwork(const InferenceEngine::CNNNetwork& network,
    ...
    OV_ITT_SCOPED_TASK(itt::domains::ArmGpuPlugin, "Plugin::QueryNetwork");
...
InferenceEngine::Parameter Plugin::GetMetric(const std::string& name,
    ...
    if (METRIC_KEY(SUPPORTED_METRICS) == name) {
        ...
        std::string name = "ArmGpu Device Full Name";
        ...
        // TODO: fill actual list of supported capabilities: e.g. ArmGpu device supports only FP32
        std::vector<std::string> capabilities = {METRIC_VALUE(FP32) /*, ARM_GPU_METRIC_VALUE(HARDWARE_CONVOLUTION)*/};
...
static const InferenceEngine::Version version = {{2, 1}, CI_BUILD_NUMBER, "ov_arm_gpu_plugin"};
